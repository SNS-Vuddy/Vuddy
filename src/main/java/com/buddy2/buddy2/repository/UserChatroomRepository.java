...
public interface UserChatroomRepository extends JpaRepository<UserChatroom, Long> {
    ...
    @Query(value = "SELECT DISTINCT uc.chat_id FROM user_chatroom uc INNER JOIN user_chatroom uc2 ON uc.chat_id = uc2.chat_id WHERE (uc.user_id = :userId1 AND uc2.user_id = :userId2) OR (uc.user_id = :userId2 AND uc2.user_id = :userId1)", nativeQuery = true)
    ...
    Long findwithChatIdAndUserId(Long chatId, Long userId);
}
